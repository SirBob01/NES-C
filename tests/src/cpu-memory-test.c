...
static char *test_map_ppu_registers() {
    ...
    mu_assert("PPU CTRL",
              &emu->ppu->ctrl == get_memory_cpu(emu->cpu, PPU_REG_CTRL));
    mu_assert("PPU MASK",
              &emu->ppu->mask == get_memory_cpu(emu->cpu, PPU_REG_MASK));
    mu_assert("PPU STATUS",
              &emu->ppu->status == get_memory_cpu(emu->cpu, PPU_REG_STATUS));
    mu_assert("PPU OAM ADDR",
              &emu->ppu->oam_addr == get_memory_cpu(emu->cpu, PPU_REG_OAMADDR));
    mu_assert("PPU OAM DATA",
              &emu->ppu->oam_data == get_memory_cpu(emu->cpu, PPU_REG_OAMDATA));
    mu_assert("PPU SCROLL",
              &emu->ppu->scroll == get_memory_cpu(emu->cpu, PPU_REG_SCROLL));
    mu_assert("PPU ADDR",
              &emu->ppu->addr == get_memory_cpu(emu->cpu, PPU_REG_ADDR));
    mu_assert("PPU DATA",
              &emu->ppu->data == get_memory_cpu(emu->cpu, PPU_REG_DATA));
    mu_assert("PPU OAM DMA",
              &emu->ppu->oam_dma == get_memory_cpu(emu->cpu, PPU_REG_OAMDMA));
